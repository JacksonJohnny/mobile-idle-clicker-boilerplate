...
src/
  ...
  services/        Save, migrações, settings, storage
  ui/              Builders Phaser (sem regras de compra) + feedback + tap HUD
```
...
- Volta à aba: modal “Welcome back” só se **gain > 0** e ausente ≥ **1s**.
...
Extensões naturais: missões via arquivo futuro sugerido `src/lib/objectivesEngine.js` (ainda não existe).
